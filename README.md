...
Pour le rendu : `TP_reco.ipynb` est le notebook rendu (version 2 : système retenu (30, 10), à égalité avec le meilleur NDCG à un écart-type près puis le plus varié), `TP_reco_execute.html` son export exécuté avec les sorties, `tp_reco.py` tout le code du TP en un seul script (généré à partir du notebook, à régénérer si le notebook change), et `rendu/` le dossier prêt à zipper (script, notebook, export, rapport, données). `TP_reco_complements.ipynb` est le notebook d'appui qui a servi à construire la version 2 (réglages à petit k, seuil de similarité, choix à un écart-type du meilleur). `questionnaire/v2/` contient le rapport rendu et ses figures, `questionnaire/v1/` l'ancienne version (système (50, 15)) : notebook `TP_reco_old.ipynb`, export, rapport et figures. `resultats/` garde les mesures de la validation croisée (CSV) et les figures.
