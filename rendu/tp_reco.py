...
DATA_DIR = next((d for d in (Path("Ressources/dataset"), Path("dataset"), Path("data"), Path("."))
                 if (d / "ratings_small.csv").exists()), Path("."))
...
montrer(doublons.loc[doublons["id"].isin([105045, 4912]),
...
print("Films préférés"); montrer(avec_titres(preferes))
print("Recommandations (min_voisins = 1)"); montrer(avec_titres(reco.recommander(user)))
...
print(f"Recommandations (min_voisins = {MIN_VOISINS})"); montrer(avec_titres(reco.recommander(user)).round(2))
...
montrer(pd.DataFrame([(nom, *rmse_mae(test_eval["rating"], p)) for nom, p in reperes.items()],
...
montrer(pd.DataFrame({"RMSE": par_tranche.apply(lambda e: np.sqrt((e ** 2).mean())), "avis": par_tranche.size()}).round(3).T)
...
print("NDCG@10 (moyenne des 5 parts)"); montrer(resume["NDCG@10"].unstack("min_voisins").round(3))
print("RMSE (moyenne des 5 parts)"); montrer(resume["RMSE"].unstack("min_voisins").round(3))
...
resume = pd.concat([grille, crete, complement]).groupby(["k", "min_voisins"]).mean(numeric_only=True).drop(columns="part")
...
ax.scatter(*r.loc[[(30, 10)], ["films distincts", "NDCG@10"]].to_numpy().T, facecolor="none", edgecolor="#D62728", s=260, marker="*", linewidths=1.8, label="réglage retenu (30, 10)")
for k, m in [(50, 15), (30, 10), (25, 8), (100, 35), (100, 3), (50, 3), (100, 50), (10, 3)]:
    ax.annotate(f"({k}, {m})", (r.loc[(k, m), "films distincts"], r.loc[(k, m), "NDCG@10"]), fontsize=9, xytext=(6, 4), textcoords="offset points")
plt.colorbar(sc, label="popularité médiane des films proposés (avis)")
ax.set(xlabel="films différents proposés (validation)", ylabel="NDCG@10 (validation)", title="Un point par réglage (k, min_voisins) : qualité du top 10 contre variété")
ax.legend(loc="lower right"); ax.grid(alpha=0.25); plt.tight_layout()
...
complement = pd.read_csv("resultats/v2_grille_complement_cv.csv")
par = pd.concat([grille, crete, complement]).groupby(["k", "min_voisins"]).agg(
    **{"NDCG@10": ("NDCG@10", "mean"), "NDCG écart-type": ("NDCG@10", "std"), "RMSE": ("RMSE", "mean"), "films distincts": ("films distincts", "mean")})
best = par["NDCG@10"].idxmax(); marge = par.loc[best, "NDCG écart-type"]
candidats = par[par["NDCG@10"] >= par.loc[best, "NDCG@10"] - marge]
meilleur_k, meilleur_min = candidats["films distincts"].idxmax()
print(f"Meilleur NDCG : {best} ({par.loc[best, 'NDCG@10']:.3f} ± {marge:.3f}) ; indépartageables : {list(candidats.index)}")
...
montrer(pd.DataFrame({
    ...
    "(50, 15), maximum du NDCG": evaluer_modele(RecoUserBased(k=50, min_voisins=15).fit(train), test, SEUIL_PERTINENT, TOP_K),
}).round(3))
...
if FICHIER_COMPLEMENT.exists():
    ...
    parts = parts_validation_croisee(train, N_PARTS, SEUIL_PERTINENT, RANDOM_STATE)
    complement = pd.DataFrame([{"k": k, "min_voisins": m, "part": i,
                                **evaluer_modele(RecoUserBased(k=k, min_voisins=m).fit(train_fit), valid, SEUIL_PERTINENT, TOP_K)}
                               for k, m in COMPLEMENT for i, (train_fit, valid) in enumerate(parts)])
    complement.to_csv(FICHIER_COMPLEMENT, index=False)

par_reglage = pd.concat([grille, crete, complement]).groupby(["k", "min_voisins"])
resume = par_reglage.mean(numeric_only=True).drop(columns="part")
resume["NDCG écart-type"] = par_reglage["NDCG@10"].std()
montrer(resume.loc[[(20, 5), (20, 7), (20, 8), (20, 10), (25, 8), (25, 10), (30, 10), (50, 15)],
           ["RMSE", "NDCG@10", "NDCG écart-type", "précision@10", "rappel@10", "popularité médiane", "films distincts"]].round(3))
...
for k, m in [(30, 10), (25, 8), (20, 7)]:
    ...
    lignes.append({"réglage": f"({k}, {m})", "écart NDCG moyen": d.mean(), "parts où (50, 15) est meilleur": int((d < 0).sum()),
...
resume_tout = pd.concat([grille, crete, complement]).groupby(["k", "min_voisins"]).agg(
    **{"NDCG@10": ("NDCG@10", "mean"), "NDCG écart-type": ("NDCG@10", "std"), "RMSE": ("RMSE", "mean"),
       "films distincts": ("films distincts", "mean"), "popularité médiane": ("popularité médiane", "mean")})
best = resume_tout["NDCG@10"].idxmax()
marge = resume_tout.loc[best, "NDCG écart-type"]
candidats = resume_tout[resume_tout["NDCG@10"] >= resume_tout.loc[best, "NDCG@10"] - marge].sort_values("films distincts", ascending=False)
print(f"Meilleur NDCG : {best}, {resume_tout.loc[best, 'NDCG@10']:.3f} ± {marge:.3f}  ->  indépartageables : {list(candidats.index)}")
...
class RecoSeuil(RecoUserBased):
    ...
    def __init__(self, min_sim=0.0, k=50, **kw):
        super().__init__(k=k if k else 10**6, **kw); self.min_sim = min_sim
    def voisins(self, user):
        v = self.sim.loc[user].nlargest(self.k); return v[v > self.min_sim]

SEUILS = {"référence (50, 15), seuil 0":        dict(k=50,   min_voisins=15, min_sim=0.0),
...
if FICHIER_SEUIL.exists():
    ...
    parts = parts_validation_croisee(train, N_PARTS, SEUIL_PERTINENT, RANDOM_STATE)
    seuil = pd.DataFrame({nom: pd.concat([evaluer_modele(RecoSeuil(**c).fit(t), v, SEUIL_PERTINENT, TOP_K) for t, v in parts], axis=1).mean(axis=1)
                          for nom, c in SEUILS.items()}).T
...
print("similarité du 1er / 15e / 50e voisin (min / médiane / max selon l'utilisateur) :",
      " ; ".join(f"n°{p+1} : {tri[:, p].min():.2f} / {np.median(tri[:, p]):.2f} / {tri[:, p].max():.2f}" for p in (0, 14, 49)))
for s in (0.05, 0.10, 0.15, 0.20):
    n = (S > s).sum(axis=1)
    print(f"seuil {s:.2f} : de {n.min()} à {n.max()} voisins selon l'utilisateur (médiane {int(np.median(n))}), "
...
verif_test = pd.DataFrame({f"({k}, {m})": evaluer_modele(RecoUserBased(k=k, min_voisins=m).fit(train), test, SEUIL_PERTINENT, TOP_K) for k, m in VERIF})
...
lignes = []
for k, m in VERIF:
    listes_km = {u: list(RecoUserBased(k=k, min_voisins=m).fit(train).recommander(u, TOP_K).index) for u in test["userId"].unique()}
    hits = [f for u, l in listes_km.items() for f in l if f in pert.get(u, set())]
    lignes.append({"réglage": f"({k}, {m})", "films distincts proposés": len({f for l in listes_km.values() for f in l}),
                   "bons films trouvés": len(hits), "bons films distincts": len(set(hits)),
...
validation = pd.DataFrame({
    ...
    "centré, seuil 7": moyenne_validation(RecoUserBased, k=meilleur_k, min_voisins=7, centrer=True),   # le seuil a été réglé pour le brut
    "centré, seuil 13": moyenne_validation(RecoUserBased, k=meilleur_k, min_voisins=13, centrer=True),
    "films les plus notés": pd.concat([evaluer_populaire(t, v, SEUIL_PERTINENT, TOP_K) for t, v in parts], axis=1).mean(axis=1),   # repère sans modèle
})
...
montrer(pd.DataFrame({"brut": evaluer_modele(reco_retenu, test, SEUIL_PERTINENT, TOP_K),
...
def similarites(X):
    ...
    norme = np.linalg.norm(X0, axis=1, keepdims=True); norme[norme == 0] = 1.0
    sim = (X0 @ X0.T) / (norme @ norme.T)
...
axes[0].hist(brut, bins=bins, color="#2F5BEA"); axes[0].set(title=f"Sans centrage : tout est positif (médiane {np.median(brut):.2f})", xlabel="similarité cosinus", ylabel="paires d'utilisateurs")
axes[1].hist(centre, bins=bins, color="#7A97FF"); axes[1].axvline(0, color="#444", ls="--", lw=1)
axes[1].set(title=f"Avec centrage : {(centre < 0).mean():.0%} des paires deviennent négatives", xlabel="similarité cosinus")
plt.tight_layout()
...
colonnes = {"films les\nplus notés": validation["films les plus notés"], "user-based\nbrut (retenu)": validation["brut"],
            "user-based\ncentré": validation["centré"], "item-based\npar note": validation_item["note, min_avis 5, n 10, k 10"],
            "item-based\npar somme": validation_item[validation_item.loc["NDCG@10"].idxmax()]}   # le meilleur réglage item-based
comp = pd.DataFrame(colonnes)
fig, axes = plt.subplots(1, 3, figsize=(14, 4.2))
for ax, ligne, titre in zip(axes, ["NDCG@10", "RMSE", "popularité médiane"],
                            ["NDCG@10 (plus haut = mieux)", "RMSE (plus bas = mieux)", "Popularité médiane des films proposés (plus bas = moins de biais)"]):
    vals = comp.loc[ligne]
    barres = ax.bar(vals.index, vals.fillna(0), color=["#9aa5b1", "#2F5BEA", "#7A97FF", "#F2A43A", "#D97A0B"])
    ax.bar_label(barres, labels=[f"{v:.3f}" if ligne != "popularité médiane" else f"{v:.0f}" for v in vals.fillna(0)], padding=3, fontsize=9)
    ax.set(title=titre); ax.tick_params(axis="x", labelsize=8.5); ax.grid(axis="y", alpha=0.25)
axes[1].set_ylim(0.85, 1.05); axes[1].annotate("pas de note prédite", (0, 0.935), ha="center", fontsize=8.5, color="#666")
axes[2].axhline(3, color="#444", ls="--", lw=1); axes[2].annotate("film médian du catalogue : 3 avis", (-0.4, 8), fontsize=8, color="#444")
plt.tight_layout()
...
montrer(pd.DataFrame({"user-based brut": evaluer_modele(reco_retenu, test, SEUIL_PERTINENT, TOP_K),
...
repartition = pd.DataFrame({"catalogue": par_tranche(n_avis.to_numpy()),
...
ax.set(ylabel="part (%)", xlabel="popularité du film : nombre d'avis en train", title="Où se situent les films proposés par rapport au catalogue ?")
...
montrer(pd.DataFrame({**{nom: diversite_moyenne(l) for nom, l in systemes.items()},
...
print("Films préférés"); montrer(avec_genres(preferes[u]))
print("Top 10 user-based"); montrer(avec_genres(listes_ub[u]))
...
fig, axes = plt.subplots(1, 2, figsize=(11, 4))
for ax, j, titre in ((axes[0], 0, "Genres différents couverts par un top 10"), (axes[1], 1, "Part des paires de films sans genre commun")):
    ax.boxplot([v[:, j][~np.isnan(v[:, j])] for v in valeurs.values()], showfliers=False)
    ax.set(xticks=range(1, 5), xticklabels=[n.replace(" de l'utilisateur", "\nde l'utilisateur") for n in valeurs], title=titre); ax.grid(axis="y", alpha=0.25)
plt.tight_layout()
...
montrer(avec_titres(plus_proposes.to_frame("utilisateurs")).assign(avis_en_train=lambda d: n_avis.reindex(d.index.get_level_values(0)).to_numpy()))
...
axes[0].barh(y - 0.2, [comptes_ib.get(f, 0) / len(utilisateurs) * 100 for f in top15.index], 0.4, color="#D97A0B", label="item-based, mêmes films")
axes[0].set(yticks=y, yticklabels=titres.reindex(top15.index).fillna("?"), xlabel="part des utilisateurs qui reçoivent ce film (%)", title="Les 15 films les plus proposés")
axes[0].legend(loc="lower right"); axes[0].tick_params(axis="y", labelsize=8)
...
axes[1].axhline(50, color="#999", ls=":"); axes[1].set(xscale="log", xlabel="films, du plus proposé au moins proposé (log)", ylabel="part cumulée des recommandations (%)", title="Concentration")
...
def part_genres(films):
    g = [genres_film.get(f) for f in films]; g = [x for x in g if isinstance(x, set)]
    return pd.Series([genre for ens in g for genre in ens]).value_counts() / len(g) * 100
genres = pd.DataFrame({"catalogue": part_genres(train["movieId"].unique()),
                       **{nom: part_genres([f for l in listes.values() for f in l]) for nom, listes in systemes.items() if nom != "populaire"}}).fillna(0)
...
ax.invert_yaxis(); ax.set(xlabel="part des films qui ont ce genre (%)", title="Quels genres sont proposés, comparés au catalogue ?"); ax.grid(axis="x", alpha=0.25); plt.tight_layout()
...
montrer(pd.DataFrame({f"utilisateur {u_pour}": avec_genres(listes_ub[u_pour])["titre"].to_numpy(),
...
fig, ax = plt.subplots(figsize=(9, 4)); w = 0.27
for i, ((nom, l), c) in enumerate(zip(systemes.items(), ["#2F5BEA", "#D97A0B", "#444"])):
    ...
    ax.bar(np.arange(11) + (i - 1) * w, np.bincount(communs, minlength=11) / len(communs) * 100, w, color=c, label=f"{nom} : {communs.mean():.1f} en moyenne")
ax.set(xticks=range(11), xlabel="films en commun entre les top 10 de deux utilisateurs", ylabel="part des paires (%)", title="Deux utilisateurs reçoivent-ils les mêmes films ?")
